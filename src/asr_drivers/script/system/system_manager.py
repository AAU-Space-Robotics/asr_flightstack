...
class SystemManagerNode(Node):
    def __init__(self):
        ...
        # Vehicle-specific values come from the vehicle's params.yaml:
        #   flight_stack_package: ROS package holding the bringup launch file
        #   flight_stack_launch:  launch file basename (without .launch.py)
        #   service_name:         systemd service restarted by RESTART_NODES
        self.declare_parameter('system.flight_stack_package', 'thyra')
        self.declare_parameter('system.flight_stack_launch', 'thyra')
        self.declare_parameter('system.service_name', 'thyra')
    ...
    def _start_flight_stack(self):
        package = self.get_parameter('system.flight_stack_package').get_parameter_value().string_value
        launch_file = self.get_parameter('system.flight_stack_launch').get_parameter_value().string_value
        for value in (package, launch_file):
            if not re.fullmatch(r'[a-zA-Z0-9_]+', value):
                self.get_logger().error(f'Refusing to launch invalid flight stack value: {value!r}')
                return
        subprocess.Popen(
            [self._ros2_path, 'launch', package, f'{launch_file}.launch.py'],
        )
    ...
    def execute_callback(self, goal_handle):
        ...
        if command == 'RESTART_NODES':
            service = self.get_parameter('system.service_name').get_parameter_value().string_value
            if not re.fullmatch(r'[a-zA-Z0-9_-]+', service):
                result.success = False
                result.message = f'Invalid service_name: {service!r}'
                goal_handle.abort()
                return result
            subprocess.Popen(
                [self._sudo_path, self._systemctl_path, 'restart', service],
                start_new_session=True,
            )
            result.success = True
            result.message = f'Requested {service} service restart.'
            goal_handle.succeed()
            return result
